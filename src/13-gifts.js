...
export const mostExpensive = (gifts) => {
  let expensive = gifts[0];
  for (const gift of gifts) {
    if (gift.price > expensive.price) {
      expensive = gift;
    }
  }
...
export const preferedKid = (kids) => {
  // si tableau vide → undefined
  if (kids.length === 0) return undefined;

  let favoriteKid = kids[0];
  for (const kid of kids) {
    if (totalPrice(kid.gifts) > totalPrice(favoriteKid.gifts)) {
      favoriteKid = kid;
    }
  }
  return favoriteKid.name;
};
...
export const distributeGifts = (gifts, kids) => {
  // si pas d'enfants → on touche pas aux cadeaux
  ...
  while (gifts.length > 0) {
    let leastKid = kids[0];
    for (const kid of kids) {
      const kidTotal = totalPrice(kid.gifts);
      const leastTotal = totalPrice(leastKid.gifts);
      if (kidTotal < leastTotal) {
        leastKid = kid;
      } else if (kidTotal === leastTotal && kid.behaviorScore > leastKid.behaviorScore) {
        leastKid = kid;
      }
    }
    const gift = gifts.shift();
    leastKid.gifts.push(gift);
  }
